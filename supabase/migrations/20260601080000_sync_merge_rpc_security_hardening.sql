-- Harden sync_merge_row's SECURITY DEFINER boundary.
--
-- This migration intentionally does NOT add progression_states to the generic
-- sync whitelist. The app-side progression state model is currently
-- local-only until the live Supabase schema is migrated to the same contract.
...
begin
  ...
  -- For authenticated callers, only client-writable columns may participate in
  -- the upsert. Newly added server-owned columns stay protected until they are
  -- explicitly added to the relevant allowlist above.

-- Add `workout_minute_of_day` to the sync_merge_row users writable-column
-- whitelist.
--
-- The column itself already exists (20260607120000_workout_minute_of_day.sql),
-- but sync_merge_row hardcodes a per-table writable-column whitelist for
-- non-service-role callers. Without listing `workout_minute_of_day` there, a
-- client's outboxed patch carrying the user's exact preferred workout time is
-- silently filtered out of both the INSERT and UPDATE column lists, so the
-- value never lands on the synced `users` row (it only survives on the anon
-- local doc and is copied on migration).
--
-- This is a verbatim re-statement of the 20260709120000 definition with ONLY
-- `workout_minute_of_day` added to the `users` array (right after
-- `workout_time`). RLS on public.users is already owner-only, so no policy
-- changes are needed.

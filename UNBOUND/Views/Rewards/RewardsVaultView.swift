...
struct RewardsVaultView: View {
    ...
    private func nextUpRow(_ reward: Reward) -> some View {
        HStack(spacing: 14) {
            rewardEmblem(reward, size: 44, locked: true)
    ...
    private func rewardTile(_ reward: Reward) -> some View {
        VStack(spacing: 8) {
            rewardEmblem(reward, size: 56, locked: !reward.unlocked)
            Text(reward.name)
    ...
    private var titleRewards: [Reward] {
        ...
        return TitleCatalog.all.map { tid in

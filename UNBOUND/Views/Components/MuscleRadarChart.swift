        ...
        ZStack {
            ForEach([0.25, 0.5, 0.75, 1.0], id: \.self) { scale in
                RadarGridShape(sides: data.count, scale: scale)
                    .stroke(Color.unbound.surfaceElevated, lineWidth: 0.5)
            }

            RadarDataShape(values: data.map { $0.target / 100 })
                .fill(Color.unbound.accent.opacity(0.1))
            RadarDataShape(values: data.map { $0.target / 100 })
                .stroke(Color.unbound.accent.opacity(0.4), lineWidth: 1)

            RadarDataShape(values: data.map { $0.current / 100 })
                .fill(Color.unbound.textTertiary.opacity(0.2))
            RadarDataShape(values: data.map { $0.current / 100 })
                .stroke(Color.unbound.textTertiary, lineWidth: 2)

...
extension TrainingSessionAdapters {
    ...
    static func routinePerformanceExercises(
    ...
    static func timedRoutineExercise(name: String, seconds: Int) -> PerformanceExercise {
        PerformanceExercise(
            name: cleanRoutineExerciseName(name),
            plannedSets: 1,
    ...
    static func timedRoutineSet(name: String, seconds: Int) -> PerformanceSet {
    ...
    static func instructionRoutineExercise(from text: String) -> PerformanceExercise? {
    ...
    static func performanceSets(from entry: RoutinePerformanceEntry) -> [PerformanceSet] {
    ...
    static func plannedTarget(from entries: [RoutinePerformanceEntry]) -> String {
    ...
    static func repPrescription(in text: String) -> (name: String, reps: Int)? {
    ...
    static func distanceMeters(in text: String) -> Int? {
    ...
    static func repetitionMultiplier(in text: String) -> Int? {
        let normalized = text.replacingOccurrences(of: "×", with: "x")
        guard let range = normalized.range(
            of: #"(?i)\bx\s*(\d+)\b"#,
            options: .regularExpression
    ...
    static func cleanRoutineExerciseName(_ value: String) -> String {
        var cleaned = value
    ...
    static func isRestLike(_ label: String) -> Bool {
        let lower = label.lowercased()
        return lower.contains("rest") || lower.contains("recover") || lower.contains("cool-down")
    }

    static func renumbered(_ sets: [PerformanceSet]) -> [PerformanceSet] {
...
extension PerformanceSet {
    var hasCompletedTrainingMetric: Bool {
        positiveValue(reps)
            || positiveValue(holdSeconds)
            || positiveValue(durationSeconds)
            || positiveValue(distanceMeters)
            || positiveValue(calories)
    }

    var hasCompletedSkillSessionMetric: Bool {
        positiveValue(reps) || positiveValue(holdSeconds)
    }

    private func positiveValue(_ value: Int?) -> Bool {
        (value ?? 0) > 0
    }
}

extension RoutinePerformanceEntry {

...
Current implementation note (2026-05-23, updated 2026-05-24): Binding Vows now generate real catalog-backed training drafts instead of one-off placeholder work. Low Binding and Limit Binding use multi-movement axis templates; Apex Binding drafts follow the rotating proof instead of always using the same generic circuit. Weight proofs are exercise-specific (`exerciseWeightKg`) so a vow that asks for a Bench Press proof cannot be satisfied by an unrelated heavy lift. Home owns the broad active/pickable Binding Vow card; Program only surfaces Binding Vow work when it is attached to the selected session as a modifier/add-on.
...
- **Calibration comes first when standards are unknown.** If the app does not have reliable baseline/progression data, the first generated program is a 7-day Calibration Week. It teaches the app the user's working standards at RPE 6-7 before the first real 28-day Arc.
...
- Program generation is now deterministic-first. Claude is no longer the normal source of truth for initial programs; it is an emergency fallback only. First-run users without at least two usable standards get a 7-day Calibration Week, and users with known calibration/progression data get the normal 28-day Arc.
...
- Program surface states now use a tested `ProgramSurfaceState` resolver for no program, loading, load error/retry, block complete, rest day, training day, calibration, and missing-day cases. Initial load failures without a local cache now surface a retryable Program error instead of falling through to a no-program prompt. DEBUG simulator proof can seed saved-program states with `--unbound-proof-program-state=calibration|training-day|rest-day|missing-day|block-complete` and non-program surfaces with `--unbound-proof-program-surface=no-program|loading|load-error` before opening Program.
- Block rollover now builds a tested next-block proposal from the latest scan delta when available. The block-complete UI shows proposal reasons, and build-next-block passes scan-derived focus plus stored progression state into rollover generation instead of silently ignoring it.
...
- Program Canvas integration now includes local Saved Workouts scheduling, Program-tab Saved Workouts entry, editor Save Workout, editor Add Skill Block picker, live Checkpoint flow, and per-row Wave 2 undo state.
- Checkpoint is now a skippable Program-tab journey: optional body capture, standards check, free-text notes, nutrition context, deterministic review, and next-Arc apply/skip.
- Home / Program surface overlap is now reduced: Home shows a compact launch/status card with no movement list, while Program owns the weekly calendar, selected-day exercise list, edit flow, saved workouts, checkpoint, and adaptation controls.
- Home / Profile rank-gate ownership is now explicit: Profile owns rank explanation and gate status inside the rank info plate/sheet, while Home only shows a compact Next Gate pulse when the user is ready or close. Tapping the Home pulse sends the user directly to that Profile rank sheet for the full checklist and trial start point.
- Latest Program integration verification: 105 focused Program/Checkpoint/Saved Workout/Proof simulator tests passed, full simulator suite passed 790 tests, and iPhone 17 build/install/launch smoke passed.

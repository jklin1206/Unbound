...
final class ProgramAwareLoggingTests: XCTestCase {
    ...
    func test_legacyWorkoutLoggerFlush_writesNoProgression_whileCanonicalCompleteDoes() async throws {
        // Sub-issue: DUAL LOGGERS. The legacy WorkoutLoggingViewModel flush must
        // NOT write attribute/movement/overall-level progression (that is the
        // canonical TrainingCompletionService.complete() job). It may still write
        // a compatible WorkoutLog + completion receipt.
        let progressionPrefixes = ["movement_progress/", "movement_ap_gains/", "overall_level_progress/", "body_map_profiles/"]

        // --- Legacy path ---
        let legacyServices = ServiceContainer.mock
        let viewModel = WorkoutLoggingViewModel(
            workout: workout([ex("Bench Press", sets: 1, reps: "5")]),
            programId: "legacy-program",
            dayNumber: 2,
        ...
        _ = await viewModel.saveLog()
        await viewModel.flushPendingCompletionEffects()

        let legacyDB = try XCTUnwrap(legacyServices.database as? MockDatabaseService)
        let legacyAttribute = try XCTUnwrap(legacyServices.attribute as? MockAttributeService)
        let legacyWorkoutLog = try XCTUnwrap(legacyServices.workoutLog as? MockWorkoutLogService)

        for prefix in progressionPrefixes {
            XCTAssertFalse(
                legacyDB.store.keys.contains { $0.hasPrefix(prefix) },
                "Legacy flush must not write progression collection \(prefix)"
            )
        }
        XCTAssertTrue(
            legacyAttribute.profileByUser.isEmpty,
            "Legacy flush must not ingest attribute progression"
        )
        // Receipt/history behavior preserved.
        XCTAssertEqual(legacyWorkoutLog.compatibleHistorySaveCallCount, 1)
        XCTAssertTrue(legacyDB.store.keys.contains { $0.hasPrefix("training_completion_records/") })

        // --- Canonical path (contrast) ---
        let canonicalServices = ServiceContainer.mock
        let canonicalLog = WorkoutLog(
            id: UUID().uuidString,
            userId: "mock-user-123",
            programId: "canonical-program",
            dayNumber: 1,
            ...
            exerciseEntries: [
                ExerciseLogEntry(
                    ...
                    plannedSets: 1,
                    plannedReps: "5",
                    sets: [SetLog(id: UUID().uuidString, setNumber: 1, weightKg: 100, reps: 5, rpe: 8, isWarmup: false)],
                    skipped: false,
                    notes: nil
                )
            ],
        ...
        let canonicalPerformanceLog = PerformanceLog(
            id: canonicalLog.id,
            userId: canonicalLog.userId,
            source: .program,
            title: canonicalLog.plannedWorkoutName,
            startedAt: canonicalLog.startedAt,
            completedAt: canonicalLog.completedAt ?? Date(),
            programId: canonicalLog.programId,
            dayNumber: canonicalLog.dayNumber,
            blocks: [
                PerformanceBlock(
                    kind: .strength,
                    title: canonicalLog.plannedWorkoutName,
        ...
        _ = try await TrainingCompletionService.shared.complete(canonicalPerformanceLog, services: canonicalServices)

        let canonicalDB = try XCTUnwrap(canonicalServices.database as? MockDatabaseService)
        let canonicalAttribute = try XCTUnwrap(canonicalServices.attribute as? MockAttributeService)
        XCTAssertTrue(
            canonicalDB.store.keys.contains { $0.hasPrefix("movement_progress/") },
            "Canonical complete() must write movement progression"
        )
        XCTAssertTrue(
            canonicalDB.store.keys.contains { $0.hasPrefix("overall_level_progress/") },
            "Canonical complete() must write overall-level progression"
        )
        XCTAssertFalse(
            canonicalAttribute.profileByUser.isEmpty,
            "Canonical complete() must ingest attribute progression"
        )
    }
